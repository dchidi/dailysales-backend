...
FROM
  ...
  P.ExecutiveId, U2.UserName as [Quote User],U.UserName,P.CreatedBy,CL.FirstName as ClientName,P.PetName,PC.CreatedDate as [CancellationDate],'Unknown' as [IsFlyBuy],
  PA.TotalPremiumAmount, CASE WHEN U.FirstName IN ('FIT','Web') THEN 'Web' ELSE 'Phone' END AS [PolicyReceivedMethodId],'NZ - UTS' AS [Country]
    FROM [PolicyActivity] PA
	LEFT JOIN Policy P ON P.Id = PA.PolicyId
	LEFT JOIN [Master].[PolicyStatus] PS ON PS.Id = P.PolicyStatusId
    INNER JOIN [Master].[TransactionType] TT ON TT.Id=PA.TransactionTypeId
